...
##### Edge percentage to blackout [Calibration Values]
    ...
    # Blackout edges
    img_B[:int(rows_B*edge_blackout), :, :] = 255
    img_B[:, :int(cols_B*edge_blackout), :] = 255
    img_B[int(rows_B - rows_B*edge_blackout):rows_B, :, :] = 255
    img_B[:, int(cols_B - cols_B*edge_blackout):cols_B, :] = 255
...
def Detect_Pieces(img_bin, img_c, avgArea, PuzzlePCNT):
    """
    ...
    color_e=np.array((0,255,0))
    #color = np.array([(0,0,255), (0,125,255), (0,255,255), (0,255,125), (0,255,0),(125,255,0),(255,255,0),(255,125,0),(255,0,0), (255,0,125), (255,0,255), (125,0,255),(0,0,255), (0,125,255), (0,255,255), (0,255,125), (0,255,0),(125,255,0),(255,255,0), (255,125,0), (255,0,0), (255,0,125), (255,0,255), (125,0,255)])
    color=np.tile(color_e,(24,1))
...
#       contours[h] = cnt
        cv2.drawContours(img_c,[cnt],0,[0,255,0],2)
#    showimg(img_c)
...
##### Min and Max piece areas [Calibration Values]
    ...
    piece_min_area = avgArea/2
    piece_max_area =avgArea*2
        ...
        try:
            ...
            for rho,theta in lines[0]:
                ...
                if np.shape(A_1)[0] == 0: # A_1 is empty
                    A_1.append(theta)
                elif abs((sum(A_1)/len(A_1)) - theta) <= ang_dev: # new line is extension of the previous lines
                    A_1.append(theta)

                elif np.shape(A_2)[0] == 0: # find a new line, not aligned with previous lines
                    A_2.append(theta)
                elif abs((sum(A_2)/len(A_2)) - theta) <= ang_dev: # new line, extension of previous lines
                    A_2.append(theta)
...
######### Tab width from (cx, cy) to consider [Calibration Values]
        ...
        for search in range(defects.shape[0]):
            ...
            if s<cnt.shape[0] and e<cnt.shape[0] and f<cnt.shape[0]: # defect within piece
                start = tuple(cnt[s][0])
                end = tuple(cnt[e][0])
                far = tuple(cnt[f][0])
...
def Perspective_Correction(img_ori,BoardSize):
    """
    Input: color image
    ...
    epsilon = 0.05*cv2.arcLength(border,True)
    ...
    # Calculate and return adjusted image
    img_PT = cv2.warpPerspective(img_ori, M, (cols_PT, rows_PT))
